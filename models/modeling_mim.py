...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, posi_emb=None, ad_net=None, is_source=False, teacher=False):
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states, posi_emb, ad_net, is_source=False, mask=None):
        attn_weights = []
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids, ad_net, is_source=False, mask = None):
        embedding_output, posi_emb = self.embeddings(input_ids, mask = mask)
        encoded, loss_ad, attn_weights, tran_weights = self.encoder(embedding_output, posi_emb, ad_net, is_source, mask =mask)
        return encoded, loss_ad, attn_weights, tran_weights
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False, msa_layer=12):
        ...
        self.r_phase = 0
        self.r_mag = 1.0
        self.adap_adjust_T = 1000
        self.adap_adjust_L = 4
        self.adap_adjust_append_last_subintervals= True
    ...
    def forward(self, x_s, x_t=None, ad_net=None,mask=None, teacher=False):
        ...
        if teacher:
            ...
            if x_t is not None:
                if mask is not None:
                    ...
                    x_t, loss_ad_t, _, _ = self.transformer(x_t, ad_net, mask=mask)
                    ...
                    loss_rec = (loss_rec * mask).sum() / mask.sum() / x_t.size(1)

                    #safe training
                    self.r = self.get_adjust(self.iter_num)
                    div_unique = logits_t.argmax(-1).unique().shape[0]
                    ...
                    return logits_s, logits_t, (loss_ad_s + loss_ad_t) / 2.0, loss_rec, x_s, x_t, div_unique, self.iter_num
            else:
                return logits_s, attn_s, tran_s
        ...
        with torch.no_grad():
            ...
            if posemb.size() == posemb_new.size():
                ...
                if self.classifier == "token":
                    ...
                    #vit-s/16
                    # posemb_tok, posemb_grid = posemb[:, :1], posemb[0, 0:]
