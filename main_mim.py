...
from models.modeling_mim import VisionTransformer, CONFIGS, AdversarialNetwork
from utils.scheduler import WarmupLinearSchedule, WarmupCosineSchedule
from utils.data_utils import get_loader
from utils.dist_util import get_world_size
from utils.transform_mim import get_transform
from utils.utils import visda_acc

import torch.nn.functional as F
from torchvision import transforms, datasets
from data.data_list_image_mim import ImageList, ImageListIndex, rgb_loader
...
def setup(args):
    ...
    model_ema = ModelEma(
        model,
        decay=args.model_ema_decay_init,
        device = args.device,
        resume='')
    return args, model, model_ema
...
def train(args, model, model_ema):
    ...
    # Prepare dataset
    transform_source, transform_target, transform_test = get_transform(args.dataset, args.img_size, args)
    ...
    for global_step in range(1, t_total):
        ...
        data_target = iter_target.__next__()

        x_s, y_s = tuple(t for t in data_source)
        y_s = y_s.to(args.device, non_blocking=True)

        x_s_weak, x_s_strong, x_s_mask = tuple(x_s)

        x_t, _, index_t = tuple(t for t in data_target)
        x_t_weak, x_t_strong, x_t_mask = tuple(x_t)

        #pseudo label generating
        with torch.no_grad():
            # pseudo-label with ema model
            ad_net_local.eval()
            logits_t_ema, _, _ = model_ema.ema(x_t_weak.to(args.device),ad_net=ad_net_local, teacher=True)
            probs_ema = F.softmax(logits_t_ema, dim=-1)
            score, pseudo_targets = probs_ema.max(-1)
            conf_mask = score > args.conf_threshold

        ad_net_local.train()
        logits_s, logits_t, loss_ad_local, loss_rec, x_s, x_t, div, item_num = model(x_s_strong.to(args.device), x_t_strong.to(args.device), ad_net_local, mask = x_t_mask.to(args.device))

        #pseudo-label training loss
        loss_plc = F.cross_entropy(logits_t[conf_mask], pseudo_targets[conf_mask])
        ...
        #global [cls] adversal loss
        loss_ad_global = lossZoo.adv(torch.cat((x_s[:,0], x_t[:,0]), 0), ad_net)

        #sum loss
        loss = loss_clc + args.alpha * (loss_ad_global+loss_plc) +args.beta * loss_rec+\
               args.gamma * loss_ad_local + args.deta * loss_im
        ...
        if args.local_rank in [-1, 0]:
            ...
            writer.add_scalar("train/loss_plc", scalar_value=loss_plc.item(), global_step=global_step)
            writer.add_scalar("train/loss_rec", scalar_value=loss_rec.item(), global_step=global_step)
            writer.add_scalar("train/lr", scalar_value=scheduler.get_last_lr()[0], global_step=global_step)
            writer.add_scalar("train/div", div, item_num)
...
def main():
    ...
    parser.add_argument("--name",  default="ad_small",
                        help="Name of this run. Used for monitoring.")
    parser.add_argument("--dataset", default="office", help="Which downstream task.")
    parser.add_argument("--source_list", default="data/office/amazon_list.txt", help="Path of the training data.")
    parser.add_argument("--target_list", default="data/office/dslr_list.txt", help="Path of the test data.")
    parser.add_argument("--test_list", default="data/office/dslr_list.txt", help="Path of the test data.")
    ...
    parser.add_argument("--beta", default=0.1, type=float,
                        help="The importance of the MIM loss")
    parser.add_argument("--gamma", default=0.1, type=float,
                        help="The importance of the local adversarial loss.")
    parser.add_argument("--theta", default=1.0, type=float)

    parser.add_argument("--deta", default=1.0, type=float,
                        help="The importance of mi loss.")
    ...
    parser.add_argument('--mask_patch_size', default=16, type=int, help='mask_patch_size')
    parser.add_argument('--model_patch_size', default=16, type=int, help='model_patch_size')
    parser.add_argument('--mask_ratio', default=0.2, type=float, help='mask_ratio')
    parser.add_argument('--model_ema_decay_init', default=0.999, type=float, help='model_ema_decay_init')
    parser.add_argument('--conf_threshold', default=0.7, type=float, help='conf_threshold')
    ...
    if args.is_test:
        ...
        args, model, model_ema = setup(args)
